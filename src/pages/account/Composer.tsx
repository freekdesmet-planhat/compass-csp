...
import { useLogActivity, useContacts, useProfiles, useCreateNotification } from '@/lib/hooks';
import { useSession } from '@/lib/session';
import { useToast } from '@/components/toast';
import { gmailConnected } from '@/lib/integrations';
import { LogInteractionModal, TaskModal } from '@/components/modals';
import { Sparkles, MailWarning, AtSign, MessageSquarePlus, Plus } from 'lucide-react';
  ...
  return (
    <div className="rounded-lg border bg-white">
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TaskModal open={taskOpen} onOpenChange={setTaskOpen} companyId={company.id} />
        <div className="p-3">
          ...
          <TabsContent value="task">
            <p className="text-sm text-muted-foreground">Create a task with a description, type, due date, priority and assignee.</p>
            <div className="mt-2 flex justify-end">
              <Button size="sm" variant="primary" onClick={() => setTaskOpen(true)}><Plus className="h-3.5 w-3.5" /> New task</Button>
            </div>
          </TabsContent>
        </div>
      </Tabs>
